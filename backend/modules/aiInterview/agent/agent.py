...
async def entrypoint(ctx: JobContext):
    ...
    def on_agent_started_speaking():
        asyncio.create_task(
            ctx.room.local_participant.publish_data(
                json.dumps({"type": "agent_status", "status": "speaking"}).encode(),
                reliable=True,
            )
        )
    ...
    def on_agent_stopped_speaking():
        asyncio.create_task(
            ctx.room.local_participant.publish_data(
                json.dumps({"type": "agent_status", "status": "listening"}).encode(),
                reliable=True,
            )
        )
    ...
    def score_voice(msg: llm.ChatMessage):
        ...
        asyncio.create_task(
            ctx.room.local_participant.publish_data(
                json.dumps({"type": "voice_score", "score": voice_score}).encode(),
                reliable=True,
            )
        )
